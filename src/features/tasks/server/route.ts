...
const app = new Hono()
  ...
  .post('/', sessionMiddleware, zValidator('json', createTaskSchema), async c => {
    ...
    const hightestPostitionTask = await databases.listDocuments(DATABASE_ID, TASKS_ID, [
    ...
    const newPosition =
      hightestPostitionTask.documents.length > 0 ? hightestPostitionTask.documents[0].position + 1000 : 1000
  ...
  .post(
    ...
    async c => {
      ...
      if (workspaceIds.size !== 1) {
        return c.json({ error: 'Нельзя перемещать задачи из разных рабочих пространств' }, 400)
      }
